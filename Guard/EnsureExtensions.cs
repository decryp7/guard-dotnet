...
namespace GuardLibrary
{
    ...
    public static class EnsureExtensions
    {
        ...
        /// <summary>
        /// Ensure the target(int type) is more then -1
        /// </summary>
        /// <param name="ensure">The target object to ensure that it is more then -1.</param>
        /// <returns>The target object</returns>
        /// <exception cref="ArgumentException">Thrown if the int value is not more then -1.</exception>
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Design",
            "CA1062:Validate arguments of public methods", MessageId = "0")]
        public static Ensure<int> IsMoreThenNegativeOne(this Ensure<int> ensure)
        {
            if (!(ensure.Target > -1))
            {
                throw new ArgumentException(string.Format(CultureInfo.InvariantCulture,
                    "The parameter {0} must be more then -1.", ensure.TargetName));
            }
